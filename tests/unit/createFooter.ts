...
registerSuite({
	...
	render: {
		'renders footer without pagination'() {
			...
			assert.strictEqual(vnode.vnodeSelector, 'div.dgrid-footer');
			assert.lengthOf(vnode.children, 1);
			assert.strictEqual(vnode.children![0].vnodeSelector, 'div.dgrid-status');
		...
		'renders footer with pagination - first page'() {
			...
			assert.strictEqual(vnode.children![0].vnodeSelector, 'div.dgrid-pagination');
			assert.lengthOf(vnode.children![0].children, 2);
			assert.strictEqual(vnode.children![0].children![0].vnodeSelector, 'div.dgrid-status');
			assert.strictEqual(vnode.children![0].children![0].text, '1 - 10 of 100 results');
			assert.strictEqual(vnode.children![0].children![1].vnodeSelector, 'div.dgrid-navigation');

			assert.lengthOf(vnode.children![0].children![1].children, 3);
			assert.strictEqual(vnode.children![0].children![1].children![0].vnodeSelector, 'span.dgrid-previous.dgrid-page-link');
			assert.strictEqual(vnode.children![0].children![1].children![1].vnodeSelector, 'span.dgrid-pagination-links');
			assert.lengthOf(vnode.children![0].children![1].children![1].children, 5);
			// more asserts
			assert.strictEqual(vnode.children![0].children![1].children![2].vnodeSelector, 'span.dgrid-next.dgrid-page-link');
		},
